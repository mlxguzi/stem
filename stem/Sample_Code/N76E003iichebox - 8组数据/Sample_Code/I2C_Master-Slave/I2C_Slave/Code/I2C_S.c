...
UINT8 xdata data_received[30], data_num = 0;
UINT8 xdata data_store0[32],i=0,j=0;
UINT8 flag = 0;
//========================================================================================================
    ...
    while (1)
		{
				...
				{			
						...
						for(	i=0;i<25;i++	)
						{
										...
										data_store0[i] 	= data_received[i+2];											//第一组数据
				...
				if( flag == 1 )                 //处理数据
				{
						for( i=0 ; i< 8 ;i++ )
						{
								ring_set_color_all_same(data_store0[3*i],data_store0[3*i+1],data_store0[3*i+2]);						
						}
						Timer0_Delay1ms(data_store0[24]);
